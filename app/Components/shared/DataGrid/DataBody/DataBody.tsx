...
export const DataBody = <T,> (props: IDataBody<T>) => {
  ...
  const EmptyComponent = ({ terminoBusqueda }: { terminoBusqueda?: string }) => {
  ...
  // ✅ Verificar si estamos en modo tabla desde actions
  const isTableMode = actions?.viewMode === "table";

  // ✅ Renderizado normal de datos
  if (isTableMode) {
    // Para modo tabla, renderizar solo los <tr> elementos
    return (
      <>
        {data.map((item, index) => (
          <React.Fragment key={`table-item-${index}`}>
            {renderDesktop(item, actions!)}
          </React.Fragment>
        ))}
      </>
    );
  ...
  // Para otros modos, usar el contenedor con divs
  return (
